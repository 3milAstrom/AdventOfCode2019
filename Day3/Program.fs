...
let isCrossing (x,y,z) (matchMap: Map<int*int,int>) =
    let key = x,y
    if matchMap.ContainsKey key
    then Some (abs(0 - x) +  abs(0 - y),((matchMap.TryGetValue key) |> snd) + z)
    else None

let plotLocalMap head (state : (int * int * int) list) =
    let lastElem = state.[ state.Length - 1]
    let newList = plotMap lastElem head
    let newState = state @ newList
    newState

let rec createMapWithCrossings (stringList: List<string>) (state : (int * int * int) list) =
    match stringList with
    | [] -> state
    | [head] -> plotLocalMap head state
    | head::tail -> plotLocalMap head state |> createMapWithCrossings tail


[<EntryPoint>]
let main argv =
    let firstWireMap =
        createMapWithCrossings (wire1.Split ',' |> List.ofSeq) [0,0,0]
        |> List.map(fun (x,y,z) -> (x,y),z )
        |> Map.ofList

    let distanes, steps =
        createMapWithCrossings (wire2.Split ',' |> List.ofSeq) [0,0,0]
        |> List.fold(fun ((dist,steps):List<int> * List<int>) elem ->
            match isCrossing elem firstWireMap with
            | Some (x,y) when x <> 0 && y <> 0 -> dist @ [x] ,steps @ [y]
            | _-> dist,steps
        ) ([],[])


    printfn "%A" (distanes |> List.min) //221
    printfn "%A" (steps |> List.min) //18542
